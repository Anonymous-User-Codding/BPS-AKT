...
def model_id_type(model_name):
    ...
    is_eid = True if 'eid' in words else False
    is_xid = True if 'xid' in words else False
    is_yid = True if 'yid' in words else False
    return is_eid, is_xid, is_yid, words[0]
...
def load_model(params):
    ...
    if model_type in {'akt'}:
        model = AKT(n_skill=params.n_skill, n_eid=params.n_eid, n_tid=params.n_tid, n_fid=params.n_fid,
                    n_xid=params.n_xid, n_yid=params.n_yid, n_blocks=params.n_block, d_model=params.d_model,
